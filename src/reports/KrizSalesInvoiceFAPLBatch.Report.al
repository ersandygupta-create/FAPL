report 50012 "Kri Sales Invoice FAPL"
{
    DefaultLayout = RDLC;
    RDLCLayout = './src/reports/KrizSalesInvoiceFAPLBatch.rdl';
    Caption = 'Sales Invoice FAPL';
        ...
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(PageLoop; Integer)
                {
                    ...
                    column(LocationPAN; '')//loc."GST Registration No.")
                    {
                    ...
                    column(CompanyAddr4; Loc.City + '-' + Loc."Post Code" + ',' + LocState)//CompanyAddr[4])
                    {
                    }
                    column(Location_State; LocState + '(' + LocCode + ')')
                    {
                    ...
                    column(CIN; 'CIN No.: ' + CompanyInfo.CompanyCINNo + ' E-Mail: ' + CompanyInfo."E-Mail")
                    {
                    }
                    ...
                    column(ShipToCode; ShipToAddRec.Code)
                    {
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        begin
                            ...
                            if DimensioNSetEntry.FindSet() then begin
                                repeat
                                    if (DimensioNSetEntry."Dimension Code" = 'BRAND') then begin
                                        DimsnsionValue.Reset();
                                        DimsnsionValue.SetRange("Dimension Code", DimensioNSetEntry."Dimension Code");
                                        DimsnsionValue.SetRange(Code, DimensioNSetEntry."Dimension Value Code");
                                        if DimsnsionValue.FindFirst() then
                                            Brand := DimsnsionValue.Name;
                                    end;
                                    if (DimensioNSetEntry."Dimension Code" = 'BUDGETTYPE') then begin
                                        DimsnsionValue.Reset();
                                        DimsnsionValue.SetRange("Dimension Code", DimensioNSetEntry."Dimension Code");
                                        DimsnsionValue.SetRange(Code, DimensioNSetEntry."Dimension Value Code");
                                        if DimsnsionValue.FindFirst() then
                                            BudgType := DimsnsionValue.Name;
                                    end;
                                    if (DimensioNSetEntry."Dimension Code" = 'CROP') then begin
                                        DimsnsionValue.Reset();
                                        DimsnsionValue.SetRange("Dimension Code", DimensioNSetEntry."Dimension Code");
                                        DimsnsionValue.SetRange(Code, DimensioNSetEntry."Dimension Value Code");
                                        if DimsnsionValue.FindFirst() then
                                            Crop := DimsnsionValue.Name;
                                    end;
            ...
            begin
                CompanyState.Reset();
                CompanyState.SetRange(Code, CompanyInfo."State Code");
                if CompanyState.FindFirst() then;
                HeadOfficeAdd := 'Regd. Office : ' + CompanyInfo.Address + CompanyInfo."Address 2" + CompanyInfo.City + ',' + CompanyInfo."Post Code" + ' ' + CompanyState.Description;// + ' ' + CompanyState.Description;
                RRLRNo := '';
                RRLRDate := 0D;
                VehicleNo := '';
                EWaybillNo := '';
                ...
                LocationState.Reset();
                LocationState.SetRange(Code, loc."State Code");
                if LocationState.FindFirst() then begin
                    LocState := LocationState.Description;
                    LocCode := LocationState."State Code (GST Reg. No.)";
                end;
                ...
                EwayDate := "Sales Invoice Header"."Eway Bill Date";
    ...
    var
        ...
        DimsnsionValue: Record "Dimension Value";
