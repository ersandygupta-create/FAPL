report 50022 "Kriz Sale Return Inv TM"
{
    DefaultLayout = RDLC;
    RDLCLayout = './src/reports/KrizSalesReturnInvoiceBatchTM.rdl';
    Caption = 'Truemart Sales Credit Note Invoice';
    ...
    dataset
    {
        dataitem("Sales Cr.Memo Header"; "Sales Cr.Memo Header")
        {
            ...
            RequestFilterHeading = 'Posted Sales Cr. Memo Header';

            column(No_SalesInvHdr; "No.")
            {
            }
            column(InvRef; InvRef) { }
            ...
            column(Duedt; "Sales Cr.Memo Header"."Due Date")
            {
            }
            column(Paytrm; "Sales Cr.Memo Header"."Payment Terms Code")
            {
            }
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(PageLoop; Integer)
                {
                    ...
                    column(CompanyInfo_GST_RegistrationNo; 'GSTIN: ' + "Sales Cr.Memo Header"."Location GST Reg. No.")
                    {
                    }
                    column(CustomerRegistrationLbl; CustomerRegistrationLbl)
                    {
                    }
                    column(CustomerPLNo; '')//Customer."EDC PL Number")
                    {
                    }
                    ...
                    column(CompanyAddr4; Loc.City + '-' + Loc."Post Code" + ',' + RecState.Description)//CompanyAddr[4])
                    {
                    }
                    ...
                    column(OrderDate_SalesInvHdr; "Sales Cr.Memo Header"."Document Date")
                    {
                    }
                    column(PLNo_Location; '')//Loc."EDC PL Number")
                    {
                    }
                    column(PLDate_Location; '')//Loc."EDC PL Validity")
                    {
                    }
                    column(FLNo_Location; '')//Loc."SG FL Number")
                    {
                    }
                    ...
                    column(DueDate_SalesInvoiceHdr; Format("Sales Cr.Memo Header"."Due Date", 0, '<Day,2>-<Month,2>-<Year4>'))
                    {
                    }
                    ...
                    column(OrderNo_SalesInvoiceHdr; "Sales Cr.Memo Header"."External Document No.")
                    {
                    }
                    ...
                    column(ShipToAddr1; ShipToAddRec.Code + '-' + ShipToAddRec.Name)//ShipToAddr[1])
                    {
                    }
                    column(ShipToAddr2; ShipToAddr[2])
                    {
                    }
                    column(ShipToAddr3; ShipToAddRec.Address)//ShipToAddr[3])
                    {
                    }
                    column(ShipToAddr4; shiptoaddrec."Address 2")//ShipToAddr[4])
                    {
                    }
                    column(ShipToAddr5; shiptoaddrec.City + ',' + ShipToAddRec."Post Code")//ShipToAddr[5])
                    {
                    }
                    ...
                    column(DocDate_SalesInvHeader; Format("Sales Cr.Memo Header"."Document Date", 0, '<Day,2>-<Month,2>-<Year4>'))
                    {
                    }
                    ...
                    column(AckDate; "Sales Cr.Memo Header"."Acknowledgement Date")
                    {
                    ...
                    dataitem(DimensionLoop1; Integer)
                    {
                        ...
                        begin
                            if RecState.get("Sales Cr.Memo Header"."Location Code") then;
                    ...
                    dataitem("Sales Cr.Memo Line"; "Sales Cr.Memo Line")
                    {
                        ...
                        column(AmtInclVAT_SalesInvLine; "Sales Cr.Memo Line".Amount)
                        {
                            AutoFormatExpression = "Sales Cr.Memo Line".GetCurrencyCode();
                            AutoFormatType = 1;
                        }
                        ...
                        begin
                            SalesCrMeLine.Reset();
                            SalesCrMeLine.SetRange("Document No.", "Sales Cr.Memo Header"."No.");
                            if SalesCrMeLine.FindFirst() then
                                Desc := SalesCrMeLine.Description;
                            No += 1;
                            NoofCasesLine := 0;
                            if "Sales Cr.Memo Line".Type = "Sales Cr.Memo Line".Type::Item then begin
                                Item.Get("Sales Cr.Memo Line"."No.");
                                // if Item."EDC SPQ" <> 0 then
                                //     NoofCasesLine := ROUND("Sales Cr.Memo Line".Quantity / Item."EDC SPQ", 1, '>')
                                // else
                                //     NoofCasesLine := Round("Sales Cr.Memo Line".Quantity, 1, '>');
                            end;
            ...
            trigger OnAfterGetRecord()
            begin
                HeadOfficeAdd := 'Regd. Office : ' + CompanyInfo.Address + CompanyInfo."Address 2" + CompanyInfo.City + ',' + CompanyInfo."Post Code" + ' E-Mail: ' + CompanyInfo."E-Mail";
                ...
                refInvNo.Reset();
                refInvNo.SetRange("Document No.", "Sales Cr.Memo Header"."No.");
                if refInvNo.Find('-') then begin
                    InvRef := refInvNo."Reference Invoice Nos.";
                    SIH.Reset();
                    sih.SetRange("No.", refInvNo."Reference Invoice Nos.");
                    if SIH.Find('-') then
                        DateRef := SIH."Posting Date";
                end;
                if (Invref = '') then
                    InvRef := "Sales Cr.Memo Header"."Kriz Reference Doc. No";
                ...
                ShipToAddRec.Reset();
                ShipToAddRec.SetRange(Code, "Sales Cr.Memo Header"."Ship-to Code");
                ...
                if Contact.Get("Sales Cr.Memo Header"."Bill-to Contact No.") then;
                // if Contact.Get("Sales Cr.Memo Header"."Ship-to Contact") then;
                IF FormatAddr.SalesCrMemoShipTo(ShipToAddr, CustAddr, "Sales Cr.Memo Header") Then;
                ...
                RRLRNo := '';//"Sales Cr.Memo Header".;
                RRLRDate := 0D;//"Sales Cr.Memo Header"."LR/RR Date";
                VehicleNo := '';//"Sales Cr.Memo Header"."Vehicle No.";
                ...
                GetLineAmount("Sales Cr.Memo Header");
                GetSalesGSTAmount("Sales Cr.Memo Header");
                GetTCSAmt("Sales Cr.Memo Header");
    ...
    var
        ...
        Desc: Text[100];
        SalesCrMeLine: Record "Sales Cr.Memo Line";
        ShiptoGSTIN: Code[20];
    ...
    begin
        ...
        if "Sales Cr.Memo Line"."Return Receipt No." <> '' then
            if SalesShipmentHeader.Get("Sales Cr.Memo Line"."Return Receipt No.") then
    ...
    local procedure GetTCSAmt(SalesInvoiceHeader: Record "Sales Cr.Memo Header";
        SalesInvoiceLine: Record "Sales Cr.Memo Line")
    var
    ...
    local procedure GetLineAmount(SalesInvoiceHeader: Record "Sales Cr.Memo Header")
    var
        RecItem: Record item;
    ...
    var
        ...
        SInvLine: Record "Sales Cr.Memo Line";
        TempSalesShptLine: Record "Return Receipt Line" temporary;
        MfgDate: Date;
        ExpiryDate: Date;
        NextLine: Integer;
        MRPLotInfo: Decimal;
    begin
        "Sales Cr.Memo Line".DeleteAll();

        SInvLine.Reset();
        SInvLine.SetRange("Document No.", DocumentNo);
        if SInvLine.FindSet() then
            repeat
                "Sales Cr.Memo Line".Init();
                "Sales Cr.Memo Line" := SInvLine;
                "Sales Cr.Memo Line".Insert();

                TempSalesShptLine.DeleteAll();
                if SInvLine.Type = SInvLine.Type::Item then begin
                    SInvLine.GetReturnRcptLines(TempSalesShptLine);
                    NextLine := 0;
                            ...
                            if ILE.FindSet() then
                                repeat
                                    ...
                                    if NextLine = 1 then begin
                                        "Sales Cr.Memo Line"."Description 2" := Ile."Lot No.";
                                        "Sales Cr.Memo Line"."Tax Group Code" := format(ExpiryDate, 0, '<Closing><Day,2>-<Month,2>-<Year4>');
                                        "Sales Cr.Memo Line"."VAT Clause Code" := format(MfgDate, 0, '<Closing><Day,2>-<Month,2>-<Year4>');
                                        // "Sales Cr.Memo Line"."EDC MRP" := MRPLotInfo;
                                        "Sales Cr.Memo Line".Modify();
                                    end else begin
                                        "Sales Cr.Memo Line".Init();
                                        "Sales Cr.Memo Line"."Document No." := SInvLine."Document No.";
                                        "Sales Cr.Memo Line"."Line No." := SInvLine."Line No." + NextLine - 1;
                                        "Sales Cr.Memo Line".Type := "Sales Cr.Memo Line".Type::" ";
                                        "Sales Cr.Memo Line"."Description 2" := Ile."Lot No.";
                                        "Sales Cr.Memo Line"."Tax Group Code" := format(ExpiryDate, 0, '<Closing><Day,2>-<Month,2>-<Year4>');
                                        "Sales Cr.Memo Line"."VAT Clause Code" := format(MfgDate, 0, '<Closing><Day,2>-<Month,2>-<Year4>');
                                        //"Sales Cr.Memo Line"."EDC MRP" := MRPLotInfo;
                                        "Sales Cr.Memo Line"."Attached to Line No." := SInvLine."Line No.";
                                        "Sales Cr.Memo Line".Insert();
                                    end;
                                until ile.Next() = 0;
