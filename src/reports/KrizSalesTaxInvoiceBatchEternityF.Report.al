report 50017 "Kriz Sales Tax Invoice Batch F"
{
    DefaultLayout = RDLC;
    RDLCLayout = './src/reports/KrizSalesTaxInvoiceBatchEternityF.rdl';
    Caption = 'Eternity Sales Tax Invoice (Batch Detials)- Freight';
        ...
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(PageLoop; Integer)
                {
                    ...
                    column(IFSCCode; CompanyInfo1."SWIFT Code")
                    {

                    }

                    column(SwiftCode; CompanyInfo.IBAN) { }
                    ...
                    column(LocationPAN; CompanyInfo1."P.A.N. No.")//loc."GST Registration No.")
                    {
                    ...
                    column(OrderDate_SalesInvHdr; "Sales Invoice Header"."PO Date")
                    {
                    }
                    ...
                    column(ShipToAddr1; ShipToAddRec.Code + '-' + ShipToAddr[1])
                    {
                    }
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        column(PickUpGST; PickUpGST) { }
                        column(PickupPin; PickupPin) { }
                        column(PickUpCity; PickUpCity) { }
                        column(PickUpState; PickUpState) { }
                        column(DropupPin; DropupPin) { }
                        column(DropUpCity; DropUpCity) { }
                        column(DropUpGST; DropUpGST) { }
                        column(DropUpState; DropUpState) { }
                        column(LRNo; LRNo) { }
                        column(LRDate; LRDate) { }
                        column(DlvDate; DlvDate) { }
                        column(TruckNum; TruckNum) { }
                        column(TruckType; TruckType) { }
                        column(WeighShipment; WeighShipment) { }
                        ...
                        var
                            ...
                            MISDetails: Record "KRIZ MIS Details";
                        begin
                            No += 1;
                            NoofCasesLine := 0;
                            ...
                            //Sandeep
                            MISDetails.Reset();
                            MISDetails.SetRange("Order No", "Sales Invoice Header"."Pre-Assigned No.");
                            MISDetails.SetRange("Line No", "Line No.");
                            if MISDetails.Find('-') then begin
                                Consignor := MISDetails.Consignor;
                                Consignee := MISDetails."Consignee Name";
                                PickUpGST := MISDetails."Pick Up GST";
                                PickupPin := '';
                                PickUpState := '';
                                PickUpCity := MISDetails."Pick Up";
                                DropUpCity := MISDetails."Drop Off Location";
                                DropUpGST := MISDetails."Drop Off GST";
                                DropUpState := MISDetails.State;
                                DropupPin := MISDetails."Ship To Pin";
                                LRNo := MISDetails."LR No";
                                LRDate := MISDetails."LR Date";
                                DlvDate := MISDetails."Delivery Date";
                                TruckType := MISDetails."Truck Type";
                                TruckNum := MISDetails."Truck Number";
                                WeighShipment := MISDetails."Weight of Shipment";
            ...
            trigger OnAfterGetRecord()
            begin
                HeadOfficeAdd := 'Regd. Office : ' + CompanyInfo.Name + CompanyInfo."Name 2" + ',' + CompanyInfo.City + '-' + CompanyInfo."Post Code" + ' E-Mail: ' + CompanyInfo."E-Mail";
                RRLRNo := '';
                RRLRDate := 0D;
                VehicleNo := '';
                EWaybillNo := '';
                ...
                if RecState.get("GST Bill-to State Code") then
                    BuyerState := RecState.Description;//+ '(' + RecState."State Code (GST Reg. No.)" + ')';
